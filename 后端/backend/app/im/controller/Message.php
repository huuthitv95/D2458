...
class Message
{
  ...
  {
    $client_id = SendData::getClientIdByUid(USER_ID);
    if (!$client_id) {
        $return_data = [
            'err' => 3,
            'msg' => '与服务器连接中断，请刷新页面再试！',
        ];
    ...
    // 过滤敏感词
    $badwords = file_get_contents($_SERVER['DOCUMENT_ROOT'].'/static/badwords.txt');
    $wordsArr = explode('、',$badwords);
    if (isset($post_data['content']->text)) {
        foreach ($wordsArr as $v) {
            $post_data['content']->text = str_replace($v,'**',$post_data['content']->text);
        }
    }
    ...
    if(count($member)){
      foreach ($member as $key => $value) {
        ...
        if(($friend = Friend::where([
          ...
          $show_name = isset($db_user->nickname) ? $db_user->nickname : '--';
        }
        $user_state_obj = UserState::field('photo')->where('user_id',$value['user_id'])->find();
        $face = getShowPhoto($user_state_obj,isset($db_user->sex) ? $db_user->sex : 0,$value['user_id'],70);
...
/*	
  ...
  public function addGroup()
  {
    ...
    foreach ($post_data['users'] as $user_id) {
      ...
      $user = User::field('nickname')->get($user_id);
      $user_names .= ($user_names ? ',' : '') . $user && isset($user->nickname) ? $user->nickname : '';
    }
  ...
  public function applyGroup(){
      ...
      ->find()){
         $return_data['data'] = '已经加入群了';
         return json($return_data);
      }
      //添加申请
      $chat_member_add = ChatGroupApply::create([
          'list_id' => $post_data['list_id'],
          'invite_user_id' => $post_data['inviter_id'],
          'invite_to_user_id' =>  USER_ID,
      ]);
      //给群管理员发送通知信息
      ...
      foreach($chat_member_data_ as $key => $item){
          SendData::sendToUid($item->user_id, 'chatGroupApply',[
              'id' => $chat_member_add->id,
              'user_id' =>  USER_ID,
              'content' =>  User::get(USER_ID)->nickname . '通过扫描'. User::get($post_data['inviter_id'])->nickname .'分享的二维码申请加入进入群聊',
              'text' => '未处理',
              'photo' => $photo_path,
              'nickname' => $show_name,
          ]);
      ...
      $return_data['err'] = 0;
      $return_data['data'] = '申请成功，请等待管理员审核';
      return json($return_data);
  }
    /**
     * 检测Uid是否绑定clientid
    ...
    public function checkUid(){
        ...
        if(!$post_data['token']){
            $return_data['err'] = 1;
            $return_data['msg'] = 'token不能为空';
            return json($return_data);
        }
        $db_data = System::where('key','JWT')->select()->toArray();
        Jwt::$key = $db_data[0]['value']['key']['value'];
        Jwt::$timeNum = $db_data[0]['value']['time']['value'];
        $token = Jwt::verifyToken($post_data['token']);
        $token_err = $token ? 0 : 1;
        if($token_err){
            $return_data['err'] =1;
            $return_data['msg'] = 'token error';
        } else {
            $client_id = SendData::getClientIdByUid($token['user_id']);
            $return_data['err'] = 0;
            $return_data['msg'] = 'success';
            $return_data['data'] = [
