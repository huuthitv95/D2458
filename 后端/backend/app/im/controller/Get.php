			...
			foreach ($db_data as $key => $value) {
				switch ($value['type']) {
					case 0:
						...
						$photo_path = getShowPhoto($user_state,isset($db_user->sex) ? $db_user->sex : '',$friend_id,70);
						break;
					case 1:
						/** 群聊 */
		...
		if(!isset($post_data['friend']) || $post_data['friend']){
			...
			if(count($db_data)){
				foreach ($db_data as $key => $value) {
					...
					$face = getShowPhoto($user_state_obj,isset($user->sex) ? $user->sex : '',$value['friend_id'],50);
	...
	public function chatData()
	{
		...
		switch ($db_chat_list->type) {
			case 0:
				...
				$user = User::get($obj_id);
				$show_name = $db_friend_data->remarks ? $db_friend_data->remarks : isset($user->nickname) ? $user->nickname : '';
				$is_action = 1;
				break;
			case 1:
				/** 显示群聊，群的名称 */
		...
		foreach ($group_data as $item) {
			...
			if($chat_member_data){
				...
				foreach ($group_apply_data as $value) {
	                ...
	                $inviter = User::get($value->invite_user_id)? User::get($value->invite_user_id)->nickname : '';
					$user = User::get($value->invite_to_user_id) ? User::get($value->invite_to_user_id)->nickname : '';
					$return_data['data'][] = [
						'id' => $value->id,
						'user_id' => $value->invite_to_user_id,
						'content' => $inviter . ' 邀请 ' . $user. ' 进入',
