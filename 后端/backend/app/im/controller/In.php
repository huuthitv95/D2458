...
class In
{
	public function login()
	{
		...
		if(!isset($post_data['username']) || !isset($post_data['password']) || $post_data['username'] == '' || $post_data['password'] == ''){
			$return_data['msg'] = api_msg('login.data_error');
		...
		if(!$user || $user->password !== md5($post_data['password'])){
			$return_data['msg'] = api_msg('login.bad_password');
			return json($return_data);
		}
        //var_dump($user);exit;
		if($user->status > 0){
			$status_texts = api_locale() === 'en' ? ['locked', 'frozen'] : ['khóa', 'đóng băng'];
			$return_data['msg'] = api_msg('login.locked', ['status' => $status_texts[$user->status]]);
			return json($return_data);
		}
		...
		$return_data['err'] = 0;
		$return_data['msg'] = api_msg('login.success');
		$return_data['data'] = [
	...
	public function reg()
	{
		...
		if(!isset($post_data['username']) || !isset($post_data['password']) || $post_data['username'] == '' || $post_data['password'] == ''){
			$return_data['msg'] = api_msg('login.register_data_error');
			return json($return_data);
		}

		if (!preg_match("/^\w{1,20}$/", $post_data['username'])) {
			$return_data['msg'] = api_msg('login.username_rule');
			return json($return_data);
        }
        if (!preg_match('/^[\x7f-\xff]+$/', $post_data['nickname'])) {
            $return_data['msg'] = api_msg('login.nickname_rule');
            return json($return_data);
        }

		if (!preg_match("/^\w{1,20}$/", $post_data['password'])) {
			$return_data['msg'] = api_msg('login.password_rule');
			return json($return_data);
		}

		$db_data = User::where('username', $post_data['username'])->find();
		if($db_data){
			$return_data['msg'] = api_msg('login.username_exists');
			return json($return_data);
		}
		if($user = User::create([
			...
			/** 这里注册成功后，自动添加客服为好友 */
            /** 查找客服*/
			$list = User::where("is_customer_service = 1")->select();
			...
			$return_data['err'] = 0;
			$return_data['msg'] = api_msg('login.register_success');
			$return_data['data'] = [
				'token' => self::createToken($user->id),
			];
		}
