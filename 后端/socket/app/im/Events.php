       ...
       /** 这里验证token的合法性，不通过就断开这个socket连接 */
        if($data['action'] == 'checkToken') {
            ...
            if($token_err){
                ...
                $client_id_new = Gateway::getClientIdByUid($token['user_id']);
                SendData::sendToClient($client_id,'bindUid',[ 'user_id'=> $token['user_id'].'--'.$client_id.'--'.$client_id_new[0]]);
                echo '绑定Uid---用户：'.$token['user_id'].'---uid:'.$client_id.'--'.$client_id_new[0]."\r\n";
            }
            return;
        }
       echo 'closeClient-'.$client_id."-\r\n";
