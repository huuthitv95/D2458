<template>
	<view class="page" style="overflow: hidden;">
			...
			>
				...
				<view class="row" v-for="(row, index) in msgList.list" :key="index" :id="'msg' + row.msg.id">
					...
					<block v-if="row.type == 1">
						<view class="system">
							<!-- 文字消息 -->
							<view v-if="row.msg.type == 0" class="text">{{ row.msg.content.text }}</view>
							<!-- 领取红包消息 -->
					...
					<block v-if="row.type == 0 && (index == 0 || row.msg.time - msgList.list[index - 1].msg.time > 300)">
						<view class="system" style="margin-bottom: 10upx;">
							<!-- 文字消息 -->
							<view class="text" style="background: #F4F5F6;color: #8f8f94;">{{ timestampFormat(row.msg.time + '') }}</view>
						</view>
					</block>
					...
					<block v-if="row.type == 0">
						...
						<view class="my" v-if="row.msg.user_info.uid == my_data.id">
							<!-- 左-消息 -->
							<view class="left" @longtap="msgAction(row.msg.id + '', row.msg.user_info.uid + '', row.msg.content)">
								<!-- 文字消息 -->
								<view v-if="row.msg.type == 0" class="bubble"><rich-text :nodes="row.msg.content.text" /></view>
								<!-- 语言消息 -->
								...
								<view v-if="row.msg.type == 3" class="bubble img">
									<video :src="staticPath + row.msg.content.url" autoplay="true" objectFit="cover" style="max-height: 200uxp !important;" :lazy-load="true" />
								</view>
								<!-- 红包 -->
							...
							<!-- 右-头像 -->
							<view class="right" @tap="goDetails(row.msg.user_info.uid)"><image :src="myPhoto" :lazy-load="true" /></view>
						</view>
						...
						<view class="other" v-if="row.msg.user_info.uid != my_data.id">
							...
							<!-- 右-用户名称-时间-消息 -->
							<view class="right" @longtap="msgAction(row.msg.id + '', row.msg.user_info.uid + '',row.msg.content)">
								<!-- 文字消息 -->
								<view v-if="row.msg.type == 0" class="bubble"><rich-text :nodes="row.msg.content.text" /></view>
								<!-- 语音消息 -->
								<view v-if="row.msg.type == 1" class="bubble voice" @tap="playVoice(row.msg)" :class="playMsgid == row.msg.id ? 'play' : ''">
		...
		<view class="popup-layer" :class="popupLayerClass" @touchmove.stop.prevent="discard">
			...
			<swiper class="emoji-swiper" :class="{ hidden: hideEmoji }" indicator-dots="true" duration="150">
				<swiper-item v-for="(page, pid) in emojiList" :key="pid">
					<view v-for="(em, eid) in page" :key="eid" @tap="addEmoji(em)"><image :src="'/static/img/emoji/' + em.url" :lazy-load="true" /></view>
				</swiper-item>
			</swiper>
			<!-- 更多功能 相册-拍照-红包 -->
		...
		<view class="input-box" v-if="msgList.is_msg">
			<view style="text-align: center !important;width:750upx;line-height: 100upx;border-top: #c9c9c9 solid 2upx;">禁 言 中 ...</view>
		</view>
		...
		<view class="input-box" :class="popupLayerClass" v-else>
			...
			<view class="textbox">
				...
				<view class="text-mode" :class="isVoice ? 'hidden' : ''">
					<view class="box">
						<textarea auto-height="true" v-model="textMsg" @focus="textareaFocus" :maxlength="-1" :show-confirm-bar="false" style="max-height: 190upx;overflow:auto" />
					</view>
		...
		<!-- 		<view class="go-url" v-if="msgList.type == 1">
			<view class="go-url-btn"  @tap="goPath('../push/web?url=www.qq.com')"><image src="/static/theme/default/url.png" :lazy-load="true" /></view>
			<view class="go-url-btn"  @tap="goPath('../push/web?url=www.baidu.com')"><image src="/static/theme/default/url.png" :lazy-load="true" /></view>
			<view class="go-url-btn"  @tap="goPath('../push/web?url=www.qq.com')"><image src="/static/theme/default/url.png" :lazy-load="true" /></view>
		</view> -->
	</view>
</template>
			...
			success: res => {
				...
				let row = {
					...
					msg: {
						...
						user_info: { uid: 0, username: '大黑哥', face: '/static/img/face.jpg' },
						content: { blessing: res.data.blessing, rid: Math.floor(Math.random() * 1000 + 1), isReceived: false }
					}
				};
				this.screenMsg(row);
		...
		msgAction(id, user_id,content) {
			...
			uni.showActionSheet({
				...
				success: function(res) {
					...
					}else if (itemList[res.tapIndex] == '撤回消息') {
						_this.$httpSend({
					...
					} else if (itemList[res.tapIndex] == '打开链接') {
						uni.showModal({
						    title: '提示',
